"""
Entry point: train the standard KD baseline for a single seed.

Usage:
    python code/train_standard_kd.py --seed 42 --temperature 1.0 --alpha 0.3
"""
import argparse
import os
import sys
...
from code.distillation import train_standard_kd, TeacherFeatureCache
...
def main():
    ...
    checkpoint = safe_load_checkpoint(
        f"{config.checkpoint_dir}/teacher_best.pth", config.device
    )
    ...
    student = train_standard_kd(
        ...
        run_name=f"kd_seed{args.seed}",
        teacher_cache=teacher_cache,
        temperature_override=args.temperature,
        alpha_override=args.alpha
    )

    logger.log(f"\nStandard KD training complete for seed {args.seed}.")
